...
namespace IgniteModule
{
    public class IgniteSlider : IgniteGUIElement, IPointerClickHandler
    {
        ...
        [SerializeField]
        RectTransform handleAreaRect = null;
        ...
        public static IgniteSlider Create(
            Action<float> onValueChanged,
            float minValue = 0f,
            float maxValue = 1f,
            bool wholeNumbers = false,
            UnityEvent<float> valueChangeEvent = null,
            float initialValue = 0f
        )
        {
            var instance = Instantiate(Resources.Load<GameObject>("IgniteGUI/Slider"))
                .GetComponent<IgniteSlider>();
            ...
            instance.slider.onValueChanged.AddListener(v =>
                instance.sliderValueText.text = v.ToString("F")
            );
    ...
    {
        // slider
        public static IIgniteGUIGroup AddSlider(
        ...
        // monitoring slider
        public static IIgniteGUIGroup AddMonitoringSlider(
            this IIgniteGUIGroup group,
            Func<float> monitor,
            float minValue = 0f,
            float maxValue = 1f
        )
        {
            ...
            var slider = IgniteSlider.Create(
                v => { },
                minValue,
                maxValue,
        ...
        )
        {
            ...
            slider.StartCoroutine(MonitoringCoroutine(valueChangeEvent, monitor));
            return group.Add(
                IgniteHorizontalGroup.Create().AddLabel(label).Add(slider) as IgniteHorizontalGroup
            );
        }
        ...
        )
        {
            ...
            slider.StartCoroutine(MonitoringCoroutine(valueChangeEvent, monitor));
            return group.Add(
                IgniteHorizontalGroup.Create().Add(slider).AddLabel(label) as IgniteHorizontalGroup
            );
        }

        // operable monitoring slider
        public static IIgniteGUIGroup AddOperableMonitoringSlider(
            this IIgniteGUIGroup group,
            Func<float> monitor,
            Action<float> onValueChanged,
            float minValue = 0f,
        ...
        static IEnumerator MonitoringCoroutine(
            UnityEvent<float> valueChangeEvent,
            Func<float> monitor
        )
        {
            while (true)
